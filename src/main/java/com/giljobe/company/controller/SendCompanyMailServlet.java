...
@WebServlet("/company/sendmail")
public class SendCompanyMailServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public SendCompanyMailServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Company company = CompanyService.companyService().searchCompanyByIdAndEmail(comId,comEmail);
		...
		if(company==null) {
			...
			String number = String.valueOf((int)(Math.random()*89999)+10000);//랜덤값을 보내고
			String content = """
			...
			request.getSession().setAttribute("authenticComNum", number);
			request.getSession().setAttribute("authenticComId",comId);
